"""
跨机分布式性能测试 —— 节点 0（master）。

在 master 节点 (10.31.10.62) 上运行：
    python tests/test_perf_cross_node_node0.py

本文件会 spawn 8 个本地进程（对应 8 张 GPU），占据全局 rank 0-7。
先在本节点运行本文件（TCP rendezvous 服务端），再在节点 1 上运行
test_perf_cross_node_node1.py。
...
# ================= 拓扑配置（两个文件唯一需要改的地方）=================
MASTER_ADDR = "10.31.10.62"     # master 节点 IP（本节点）
MASTER_PORT = "29500"           # rendezvous 端口，两节点需一致且不冲突
NODE_RANK = 0                   # 本节点在全局中的编号：master=0, worker=1
N_GPUS = 8
N_NODES = 2
...
def _make_io(world_size: int):
    ...
    # output 是本 rank 拿到的 1/world_size 分片。
...
def worker(local_rank: int):
    ...
    if dist.get_rank() == 0:
        prof.export_chrome_trace("./trace_node0.json")
